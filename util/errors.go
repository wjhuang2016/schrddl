...
func SetRCIsolation(isolation bool) {
	RCIsolation = isolation
}

func GetRCIsolation() bool {
	return RCIsolation
}
...
func DMLIgnoreError(err error) bool {
	...
	// PlanCache related errors
	if strings.Contains(errStr, "Illegal mix of collations") ||
		strings.Contains(errStr, "SQL syntax") ||
		strings.Contains(errStr, "Unsupported type: Set") ||
		strings.Contains(errStr, "Invalid data type for JSON data in argument 2 to function json_contains") ||
		strings.Contains(errStr, "Prepared statement not found") {
		return true
	}
